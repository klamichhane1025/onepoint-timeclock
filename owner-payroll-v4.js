...
function downloadCsv(rows){
 ...
 const a=document.createElement('a'),u=URL.createObjectURL(new Blob([csv],{type:'text/csv;charset=utf-8'}));a.href=u;a.download=`onepoint-payroll-${new Date().toISOString().slice(0,10)}.csv`;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(u),1000);
}
async function renderPayroll(){
 ...
 try{
  ...
  const [tr,er,sr,jr]=await Promise.all([
   sb.from('time_entries').select('*,employees(employee_number,name),stores(store_code,name),job_codes(code,name)').eq('organization_id',c.orgId).order('actual_clock_in',{ascending:false}).limit(3000),
  ...
  root.innerHTML=`<div class="card"><div class="head"><div><h2>Time & Payroll</h2><div class="muted">Choose the pay period, employees and one or more stores. Payroll uses the pay rate captured on each shift.</div></div><div class="actions compact"><button class="btn secondary" id="v4History">Voided History</button><button class="btn primary" id="v4Export">Export Payroll</button></div></div>
  ...
  <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:18px;margin-top:18px">
   <div class="card" style="padding:16px"><div class="head" style="margin-bottom:10px"><b>Employees</b><label style="font-weight:700"><input type="checkbox" id="v4AllEmployees" checked> Select All</label></div><div class="list" style="max-height:230px;overflow:auto">${emps.map(e=>`<label class="row" style="display:flex;gap:10px;align-items:center;justify-content:flex-start"><input type="checkbox" class="v4Employee" value="${e.id}" checked><span><b>${esc(e.name)}</b><span class="muted"> · #${esc(e.employee_number||'')}</span></span></label>`).join('')||'<span class="muted">No employees.</span>'}</div></div>
   <div class="card" style="padding:16px"><div class="head" style="margin-bottom:10px"><b>Stores</b><label style="font-weight:700"><input type="checkbox" id="v4AllStores" checked> Select All</label></div><div class="list" style="max-height:230px;overflow:auto">${stores.map(s=>`<label class="row" style="display:flex;gap:10px;align-items:center;justify-content:flex-start"><input type="checkbox" class="v4Store" value="${s.id}" checked><span><b>${esc(s.name||s.store_code)}</b>${s.active?'':' <span class="muted">(Inactive)</span>'}</span></label>`).join('')||'<span class="muted">No stores.</span>'}</div></div>
  ...
  function draw(){
   ...
   const perEmployee=selectedNames.map(e=>{const erows=rows.filter(x=>x.employee_id===e.id),h=erows.reduce((s,x)=>s+hoursFor(x),0),w=erows.reduce((s,x)=>s+(wageFor(x)||0),0);return `<div><b>${esc(e.name)}</b><span class="muted"> #${esc(e.employee_number||'')}</span><div>${h.toFixed(2)} hrs · ${money(w)}</div></div>`}).join('');
   ...
   <div class="card" style="margin-top:18px;padding:18px"><div class="head"><div><b>Payroll Totals</b><div class="muted">${selectedNames.length} employee${selectedNames.length===1?'':'s'} selected · ${rows.length} shift${rows.length===1?'':'s'}</div></div><div style="display:flex;gap:28px;flex-wrap:wrap"><div><span class="muted">Total Hours</span><div style="font-size:24px;font-weight:800">${totalHours.toFixed(2)}</div></div><div><span class="muted">Total Wages</span><div style="font-size:24px;font-weight:800">${money(hourlyWages)}</div></div></div></div>${monthlyRows?`<div class="notice" style="margin-top:12px">${monthlyRows} monthly-salary shift${monthlyRows===1?' is':'s are'} shown but not prorated into Total Wages.</div>`:''}${perEmployee?`<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:12px;margin-top:14px">${perEmployee}</div>`:''}</div>`;
   $$('.v4Sort').forEach(b=>b.onclick=()=>{const k=b.dataset.sort;if(sortKey===k)sortDir*=-1;else{sortKey=k;sortDir=1}draw()});
   $$('.v2Void').forEach(b=>b.onclick=()=>voidEntry(b.dataset.id));$$('.v4Restore').forEach(b=>b.onclick=()=>restoreEntry(b.dataset.id));
  }
  ...
  const syncMaster=(master,items)=>{master.checked=items.every(x=>x.checked);master.indeterminate=!master.checked&&items.some(x=>x.checked)};
  ...
  $('#v4History').onclick=()=>{showVoided=!showVoided;$('#v4History').textContent=showVoided?'Active Payroll':'Voided History';draw()};$('#v4Export').onclick=()=>{const rows=filtered();if(!rows.length)return show('There are no payroll rows to export for the current selection.','Export Payroll');downloadCsv(rows)};
  draw();
 }catch(e){show(e.message||String(e),'Time & Payroll')}
}

document.addEventListener('click',e=>{const b=e.target.closest('#nav [data-tab="timesheets"]');if(!b)return;setTimeout(renderPayroll,220)},false);
