(()=>{
  ...
  const style=document.createElement('style');style.textContent=`
    ...
    .opLivePanel{position:absolute;right:0;top:calc(100% + 10px);width:min(330px,calc(100vw - 28px));background:rgba(255,255,255,.98);border:1px solid var(--pup-line,var(--line,#e5e5e7));border-radius:16px;box-shadow:0 18px 48px rgba(0,0,0,.14);padding:10px;z-index:80;opacity:0;visibility:hidden;transform:translateY(-4px);pointer-events:none;transition:opacity .13s ease,transform .13s ease,visibility .13s ease;backdrop-filter:blur(20px)}
    .opLiveHub:hover .opLivePanel,.opLiveHub:focus-within .opLivePanel,.opLiveHub.open .opLivePanel{opacity:1;visibility:visible;transform:translateY(0);pointer-events:auto}
    ...
    @media(max-width:700px){.opLivePanel{position:fixed;right:14px;top:66px}.opLiveHub{margin-left:auto}.opLiveButton{width:36px;height:36px}}
  `;document.head.appendChild(style);

  let rows=[],loading=false,channel=null,lastLoad=0,renderTimer=null;
  ...
  function mount(){
    ...
    if(!hub){
      ...
      hub.addEventListener('mouseenter',()=>{if(Date.now()-lastLoad>10000)load(true)});
      document.addEventListener('click',e=>{if(!hub.contains(e.target)){hub.classList.remove('open');hub.querySelector('#opLiveButton')?.setAttribute('aria-expanded','false')}});
      document.addEventListener('keydown',e=>{if(e.key==='Escape'){hub.classList.remove('open');hub.querySelector('#opLiveButton')?.setAttribute('aria-expanded','false')}});
    }
