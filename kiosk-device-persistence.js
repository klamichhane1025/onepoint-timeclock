...
async function load(){
 ...
 const ca=cookieAnchor(),candidates=uniqueCandidates([local,dbAnchor,file,ca]);
 const machine_id=candidates[0]?.machine_id||keyRec?.machine_id||dbAnchor?.machine_id||file?.machine_id||ca?.machine_id||local.machine_id||'';
 ...
 if(machine_id&&machine_id!==local.machine_id)lsSet('onepoint_machine_id',machine_id);
...
async function recoverWithKey(){const s=await load(),rec=s.key_record;if(!window.crypto?.subtle||!rec?.privateKey||!rec?.machine_id)return null;try{const c=await callDevice({action:'challenge',machine_id:rec.machine_id}),sig=await crypto.subtle.sign({name:'ECDSA',hash:'SHA-256'},rec.privateKey,new TextEncoder().encode(c.challenge)),d=await callDevice({action:'verify',machine_id:rec.machine_id,challenge_id:c.challenge_id,signature:toB64url(sig)});await saveCredentials({machine_id:rec.machine_id,device_token:d.device_token,store:d.store||s.store});const after=await load();if(!after.candidates.length)try{await registerRecovery(d.device_token,rec.machine_id)}catch{}return d}catch{return null}}
async function adoptCurrentDevice(){const s=await load(),token=s.device_token||lsGet('onepoint_kiosk_token')||'';if(token.length<32)return null;let valid;try{valid=await callStatus(token)}catch{return null}const mid=s.candidates[0]?.machine_id||s.key_record?.machine_id||s.machine_id||await ensureMachineId(),existing=s.candidates.find(x=>x.machine_id===mid);await saveCredentials({machine_id:mid,device_token:token,recovery_secret:existing?.recovery_secret,store:valid.store||s.store});if(existing)return{ok:true,store:valid.store,recovery_existing:true};try{const d=await registerRecovery(token,mid);return{ok:true,store:valid.store,recovery_registered:true,recovery_secret:d.recovery_secret}}catch(e){console.warn('Recovery enrollment deferred:',e);return{ok:true,store:valid.store,recovery_deferred:true}}}
async function bootstrap(){await requestPersistentStorage();const s=await load();if(s.device_token?.length>=32){let d=null;try{d=await callStatus(s.device_token)}catch{}if(d){const mid=s.candidates[0]?.machine_id||s.key_record?.machine_id||s.machine_id||await ensureMachineId();await saveCredentials({machine_id:mid,device_token:s.device_token,store:d.store||s.store});if(!s.candidates.length)try{await registerRecovery(s.device_token,mid)}catch(e){console.warn('Recovery enrollment deferred:',e)}return d}await clearToken()}
 const keyed=await recoverWithKey();if(keyed)return keyed;return await recover()}
window.OnePointDevicePersistence={load,ensureMachineId,saveCredentials,clearToken,recover,recoverWithKey,adoptCurrentDevice,requestPersistentStorage,readDb,writeDb,readFile,writeFile};
window.onePointDeviceReady=bootstrap().catch(e=>{console.warn('OnePoint persistent-device bootstrap:',e);return null});
let tries=0;const timer=setInterval(async()=>{if(++tries>20)return clearInterval(timer);try{const s=await load();if(s.device_token?.length>=32&&!s.candidates.length){const ok=await adoptCurrentDevice();if(ok&&!ok.recovery_deferred)clearInterval(timer)}}catch{}},750);
})();