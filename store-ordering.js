(()=>{
  ...
  style.textContent=`.opSortableStore{position:relative}.opStoreDragHandle{width:28px;height:28px;min-width:28px;border-radius:8px;display:grid;place-items:center;color:var(--pup-tertiary,#86868b);font-size:17px;line-height:1;user-select:none;cursor:grab;background:rgba(118,118,128,.06);touch-action:none}.opStoreDragHandle:active{cursor:grabbing}.opSortableStore.opDragging{opacity:.48}.opStoreDragHandle:hover,.opStoreDragHandle:focus-visible{background:rgba(118,118,128,.12);color:var(--pup-text,#1d1d1f);outline:2px solid rgba(0,113,227,.24);outline-offset:2px}.opPeriodTile .opStoreDragHandle,.apTile .opStoreDragHandle{position:absolute;right:10px;top:10px;z-index:4}.list>.row.opSortableStore>.opStoreDragHandle{align-self:center;flex:0 0 28px;margin-right:3px}.opOrderHint{display:inline-flex;align-items:center;gap:6px;margin-top:5px;color:var(--pup-secondary,#6e6e73);font-size:11px}.opOrderSaved{color:#16803d;font-weight:700;opacity:0;transition:opacity .2s}.opOrderSaved.show{opacity:1}.opSortDropBefore{box-shadow:inset 0 3px 0 var(--pup-blue,#0071e3)!important}.opLocationGrid{display:grid!important;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px;align-items:stretch;margin-top:14px}.opLocationGrid>.row.opSortableStore{display:flex!important;flex-direction:column;align-items:stretch;justify-content:flex-start;gap:0;min-width:0;min-height:176px;padding:16px!important;border:1px solid var(--pup-border,#e5e5ea);border-radius:14px;background:var(--pup-surface,#fff);box-shadow:0 1px 2px rgba(0,0,0,.025)}.opLocationGrid>.row.opSortableStore>.opStoreDragHandle{position:absolute;right:12px;top:12px;z-index:3;margin:0}.opLocationGrid>.row.opSortableStore>div:first-of-type{padding-right:38px;min-width:0}.opLocationGrid>.row.opSortableStore>.actions{margin-top:auto;padding-top:14px;border-top:1px solid rgba(118,118,128,.14);justify-content:flex-start;align-items:center;flex-wrap:wrap}.opLocationGrid>.row.opSortableStore .muted{overflow-wrap:anywhere}.opLocationGrid>.row.opSortableStore.opSortDropBefore{box-shadow:inset 3px 0 0 var(--pup-blue,#0071e3),0 6px 18px rgba(0,0,0,.05)!important}@media(max-width:1120px){.opLocationGrid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:700px){.opLocationGrid{grid-template-columns:1fr}.opLocationGrid>.row.opSortableStore{min-height:0}}`;
  ...
  function addHint(mode){
    ...
    const d=document.createElement('div');d.className='opOrderHint';d.innerHTML=`<span>⋮⋮ Drag the handle to reorder locations. ${mode==='overview'?'The same order is used on Locations.':'Locations fill left to right in your saved order, and the same order is used on Overview.'}</span><span class="opOrderSaved">Order saved</span>`;h.appendChild(d)
  }
  ...
  function bind(items,id){
    const mode=activeMode(),parent=items[0]?.parentElement;
    if(path==='/owner'&&mode==='locations'&&parent)parent.classList.add('opLocationGrid');
    items.forEach(item=>{
