(()=>{
  ...
  function friendly(message){const raw=String(message||'').trim(),m=raw.toLowerCase();if(!raw)return 'Please review the information and try again.';
    ...
    if(m.includes('shared roster name conflict'))return 'An employee with that name is already visible to this Owner through an owned or shared location. Rename one of the employees before continuing. This does not create a new Employee ID or PIN—the shared employee keeps the same login.';
    ...
    if(m.includes('employees_organization_id_employee_number'))return 'That Employee ID is already in use under this Owner. Please try again so OnePoint can generate another ID.';
    if(m.includes('job_codes_org_normalized_code')||m.includes('job_codes_organization_id_code'))return 'That Job Code is already in use. Please choose a different code.';
    if(m.includes('job_codes_org_normalized_name')||m.includes('job_codes_organization_id_name'))return 'A Job Code with that name already exists. Please use a different name.';
    if(m.includes('organization_users_org_email_unique')||m.includes('duplicate')&&m.includes('email'))return 'That email address is already assigned in this organization. Please use a different email or edit the existing user.';
    if(m.includes('duplicate key value')||m.includes('unique constraint')||m.includes('23505'))return 'One of those values is already in use. Please review the fields and enter a different value.';
